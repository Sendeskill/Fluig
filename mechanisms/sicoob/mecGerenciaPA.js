		...
		for (var i = 0; i < dados.length; i++) {	
			if (dados[i].childGroupCode != null) {
				...
				if (descGrupo.toUpperCase().indexOf("CAIXA-") != -1 
					...
					var superior = buscaSuperior(codGrupo);
					userList.add('Pool:Group:' + superior);
				}
			}
		}
	}
...
function buscaSuperior(codGrupo) {
	var fields        = null;
	var constraints   = new Array();
	constraints.push(DatasetFactory.createConstraint("GRUPO", codGrupo, codGrupo, ConstraintType.MUST));
	var sortingFields = null;
	
	var dataset = DatasetFactory.getDataset('dsSuperior', fields, constraints, sortingFields);
	...
	if (count == 0) {
		...
		if (dataset.getValue(0, "MENSAGEM_RETORNO") == 'OK') {
			return dataset.getValue(0, "SUPERIOR");
		} else {
			return "";
		}
