var Alx = {
    Table: function(tablename, fieldIndex, fieldParentIndex) {
        ...
        this.add = function(element, callback = undefined) {
            var lastNumber;
            ...
            var $elementRow = $trLast.find('td').find('[alx-row]').first();
        ...
        this.render = function(callback = undefined) {
            $containerRows = $('[alx-container-rows="' + this.tablename + '"]');
            ...
            $rows.each(function () {
                    var $elementRow = $(this).find('td').find('[alx-row]').first();
            ...
            self.afterRenderAll($containerRows[0]);
