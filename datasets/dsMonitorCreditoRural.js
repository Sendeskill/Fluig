function createDataset(fields, constraints, sortFields) {
  ...
  options['_action'] = getContraintByFieldName(constraints, '_action').initialValue;
	...
	if (options['_action'] === 'createMonitorCreditoRural')
		return createMonitorCreditoRural(options);
	else
    return selectMonitorCreditoRural(options);
}

//Retona os dados para a tabela
function selectMonitorCreditoRural(options) {
  ...
  //       var myQuery = "SELECT TOP 50 * FROM [DataTransaction].[fluig].[MonitorCreditoRural] WHERE " + filtro.join(" AND ");
...
function createMonitorCreditoRural(options) {
  ...
  var OAUTH_USER_APP_PUBLIC = "f5614b89-ea0a-4ad9-ace6-ec955c595ef2";
	...
	for (var i = 0; i < data.length; i++) {	
		...
		consumer.post(FLUIG_HOST + "/bpm/api/v1/processes/monitor_credito_rural/start", body); 
	}
}
